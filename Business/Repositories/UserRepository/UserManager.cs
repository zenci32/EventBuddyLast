...
namespace Business.Repositories.UserRepository
{
    public class UserManager : IUserService
    {
        ...
        public UserManager(IUserDal userDal, IFileService fileService, IEmailService emailParameterService)
        {
            _userDal = userDal;
        ...
        public async Task<IDataResult<UserInfoDto>> GetUserInfo(string phone)
        {
            var user = await _userDal.Get(p => p.Phone == phone);
        ...
        public async Task<IDataResult<User>> GetByPhone(string phone)
        {
            return new SuccessDataResult<User>(await _userDal.Get(p => p.Phone == phone));
        }
        ...
        public async Task<IResult> Delete(User user)
        {
            ...
            return new SuccessResult(UserMessages.DeletedUser,200);
        }

        public async Task<IDataResult<List<User>>> GetList()
        {
            return new SuccessDataResult<List<User>>(await _userDal.GetAll(),"",200);
        }
        ...
        public async Task<IResult> ChangePassword(UserChangePasswordDto userChangePasswordDto)
        {
            ...
            if (!result)
            {
                return new ErrorResult(UserMessages.WrongCurrentPassword, 404);
            }
            ...
            return new SuccessResult(UserMessages.PasswordChanged,200);
        }
        ...
        public async Task<IResult> CreateANewPassword(CreateANewPasswordDto createANewPasswordDto)
        {
            ...
            if (user == null)
                return new ErrorResult(UserMessages.ForgotPasswordValueIsNotValid,404);
        ...
        public static IResult IsForgotPasswordValueUsed(User user)
        {
            if (user.IsForgotPasswordComplete)
                return new ErrorResult(UserMessages.ForgotPasswordValueIsUsed,404);
            else
        ...
        public static IResult IsForgotPasswordDateEnded(User user)
        {
            ...
            if (remainMin < -5)
            {
                return new ErrorResult(UserMessages.ForgotPasswordValueTimeIsEnded, 404);
            }
        ...
        public async Task<IResult> ConfirmUser(string confirmValue)
        {
            ...
            if (user.IsConfirm)
            {
                return new ErrorResult(UserMessages.UserAlreadyConfirm, 404);
            }

            user.IsConfirm = true;
            await _userDal.Update(user);
            return new SuccessResult(UserMessages.UserConfirmIsSuccesiful,200);
        }
        ...
        public async Task<IResult> SendForgotPasswordMail(string email)
        {
            ...
            return new SuccessResult(UserMessages.ForgotPasswordMailSendSuccessiful,200);
        }
        ...
        public IResult CheckForgotPasswordIsRequestActive(User user)
        {
            if (!user.IsForgotPasswordComplete)
            {
                if (user.ForgotPasswordRequestDate != null)
                {
                    ...
                    if (remainMin >= -5)
                    {
                        return new ErrorResult(UserMessages.AlreadySendForgotPasswordMail, 404);
                    }
                }
            }
        ...
        public async Task<IResult> SendConfirmUserMail(string email)
        {
            var user = await _userDal.Get(p => p.Email == email);
            if (user == null)
                return new ErrorResult(UserMessages.UserNotFound, 404);
